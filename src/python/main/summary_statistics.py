...
def main():
    ...
    if len(args) != 4:
        print("usage: patch | python3 diff_metrics.py <project> <bug_id> <repo_root> <diff version>")
        sys.exit(1)
    ...
    if version == "whitespace":
        ...
        for i in range(len(diff_sets)):
            ...
            patch = PatchSet.from_filename(path.join(repository, "diff", diff_file), encoding="latin-1")
            sizes[i] = count_all_changed_lines(patch)
        print(
            f"{project},{vid},{sizes[0]},{sizes[1]},{sizes[2]}"
        )
    else: # Clean version
